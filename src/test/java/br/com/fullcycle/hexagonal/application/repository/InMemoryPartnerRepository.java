...
public class InMemoryPartnerRepository implements PartnerRepository {
    ...
    public Optional<Partner> partnerOfCNPJ(String cnpj) {
        return Optional.ofNullable(this.partners.get(Objects.requireNonNull(cnpj)));
    }

    @Override
    public Optional<Partner> partnerOfEmail(String email) {
        return Optional.ofNullable(this.partners.get(Objects.requireNonNull(email)));
    }

    @Override
    public Partner create(Partner partner) {
        this.partners.put(partner.partnerId().value().toString(), partner);
        this.partners.put(partner.cnpj().value(), partner);
        this.partners.put(partner.email().value(), partner);
        return partner;
    }
